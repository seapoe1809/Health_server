...
current_dir = f"{APP_dir}/Weight_Tracker" #replace this and following code in new app
#current_dir=HS_path
# Build the source and destination file paths
source_file_path = os.path.join(current_dir, "Weight_Tracker_script.js")
destination_file_path = os.path.join(HS_path, "static", "Weight_Tracker_script.js")

def main():
    try:
        subprocess.run(["docker", "build", "-t", "darnahi_weight_tracker", "."], cwd=current_dir)
        #docker build -t darnahi_weight_tracker .
        
        subprocess.run(["docker", "run", "-d", "--restart", "unless-stopped", "--network", "host", "--name", "darnahi_weight_tracker", "-e", "OLLAMA_HOST=http://localhost:11434", "darnahi_weight_tracker"])
